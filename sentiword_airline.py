...
plt.savefig('./airlineresults/lexicon_performance.png', format='png', transparent=False)  # save result
plt.show()
...
plt.ylabel('true')
plt.savefig('./airlineresults/lexicon_confusion_matrix.png', format='png', transparent=False)  # save result
plt.show()
...
cmf = pd.DataFrame(data=cm,
                   columns=pd.MultiIndex.from_tuples([('predicted:', label) for label in labels], names=['', '']),
                   index=pd.MultiIndex.from_tuples([('actual:', label) for label in labels], names=['', '']))
print(cmf)
...
metrics.to_csv('./airlineresults/lexicon_performance_result.csv', index=None, header=True)
