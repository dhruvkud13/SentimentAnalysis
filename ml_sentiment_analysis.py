...
feature = tv.get_feature_names_out()[0:200]
...
# print(tv_cmf)
level = [list(range(len(labels))), labels]
tv_cmf = pd.DataFrame(data=tv_cm,
                      columns=pd.MultiIndex.from_tuples(zip(['predicted'] * len(labels), labels), names=['', '']),
                      index=pd.MultiIndex.from_tuples(zip(['actual'] * len(labels), labels), names=['', '']))
print(tv_cmf)
